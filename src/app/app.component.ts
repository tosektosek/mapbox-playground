...
export class AppComponent implements OnInit {
  ...
  addMarker(event) {
    ...
    if (event.lngLat && event.originalEvent.target.className === 'mapboxgl-canvas') {
      const a = JSON.parse(JSON.stringify(this.data));
      const feature = {
        'type': 'Feature',
        'geometry': {
          'type': 'Point',
      ...
      this.data = null;
      setTimeout(() => {
        // this.supercluster.points.push(feature);
  ...
  handleMarkerClick(event, feature) {
    event.stopPropagation();
    this.selectedMarker = { ...feature };
  }

  clusterClicked(coordinates) {
    this.selectedMarker = null;
    this.zoom++;
  ...
  zoomEnd(event) {
    this.zoom = event.target.transform.zoom;
  }
}
